      ...
      if (node.body.length === 0) {
        ...
        if (firstStatement.type === 'ReturnStatement') {
          ...
          if (reduced.type === 'BlockStatement') {
            ...
            const statementBodyAfterAddingUndefined = ast.blockStatement([und as es.Statement, ...(reduced.body)])
            return [
              ast.program([statementBodyAfterAddingUndefined, ...(otherStatements as es.Statement[])]),
              cont,
              path,
              str]
          } else {
