...
describe('primitive stream functions', () => {
  test('empty stream is null', () => {
    return expectResult('stream();', { chapter: 4, variant: 'lazy', native: false }).toBe(null)
  })
...
`)
  ...
  test('infinite stream is infinite', () => {
    ...
    `,
      { chapter: 4, variant: 'lazy', native: false }
...
test('for_each', () => {
  return expectResult(
    ...
    let sum = 0;
    let new_sum = 0;
    stream_for_each(x => {new_sum = sum + x; sum = new_sum;}, stream(1, 2, 3));
    sum;
  `,
    { chapter: 4, variant: 'lazy', native: false }
