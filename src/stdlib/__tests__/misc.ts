...
describe('Test regular function', () => {
  expectResultsToEqual(
    [
      // arity
      ['arity with nullary function is ok', 'arity(math_random);', 0],
      ['arity with function with parameters is ok', 'arity(arity);', 1],
      ['arity ignores the rest parameter', 'arity(display);', 1],
      [
      ...
      ['arity with user-made nullary function is ok', 'arity(() => undefined);', 0],
      [
      ...
      // char_at
      ['char_at with valid args is ok', 'char_at("123", 0);', '1'],
      [
      ...
      ['parse_int with valid args is ok, radix 36', "parse_int('uu1', 36);", parseInt('uu1', 36)],
      [
