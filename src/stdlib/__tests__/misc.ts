...
test('parse_int with valid args is ok, radix 2', () => {
  ...
  )).resolves.toBe(parseInt('1100101010101', 2))
})
...
test('char_at with non string first argument errors', () => {
  return expect(testFailure(stripIndent`
    char_at(42, 123);
  `)).resolves.toMatchInlineSnapshot(`"Line 1: Error: char_at expects the first argument to be a string."`)
})
...
test('arity with non-function arg f throws error', () => {
  ...
  `,
    { chapter: Chapter.SOURCE_1 }
  )).resolves.toMatchInlineSnapshot(`"Line 1: Error: arity expects a function as argument"`)
})
