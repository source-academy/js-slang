...
export type Stream<H,T> = [H, () => T]

export type List = null | NonEmptyList
interface NonEmptyList extends Stream<any, any> {}
...
function function_test(x:any){
  return x instanceof Function
}

// pair constructs a pair using a two-element array
// LOW-LEVEL FUNCTION, NOT SOURCE
export function pair<H, T>(x: H, xs: T): Stream<H, T> {
  return [x,()=>xs]
}
...
export function is_pair(x: any) {
  return array_test(x) && x.length === 2 && function_test(x[1])
}
