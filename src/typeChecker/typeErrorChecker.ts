        ...
        case 'TSTypeAliasDeclaration':
          ...
          if (bodyNode.typeParameters && bodyNode.typeParameters.params.length > 0) {
            ...
            bodyNode.typeParameters.params.forEach(param => {
              ...
              if (
                Object.values(typeAnnotationKeywordToBasicTypeMap).includes(name as TSBasicType)
              ) {
    ...
    switch (expectedType.kind) {
      case 'pair':
        ...
        if (actualType.kind === 'pair') {
          mappings.push(
            ...getTypeVariableMappings(node, actualType.headType, expectedType.headType)
          )
          mappings.push(
            ...getTypeVariableMappings(node, actualType.tailType, expectedType.tailType)
          )
        }
        break
      case 'list':
