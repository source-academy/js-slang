...
// The line & column numbers of each node in the ASTs derived from parsing the
// pre-transform code & the equivalent post-transform code will not be the same.
...
const stripCharacterOffsets = (node: es.Node): es.Node => {
...
describe('transformImportedFile', () => {
  it('wraps the entire file in a FunctionDeclaration', () => {
    ...
    expect(stripCharacterOffsets(transformedProgram)).toEqual(
      stripCharacterOffsets(expectedProgram)
    )
  })
})
