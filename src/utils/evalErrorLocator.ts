...
export function getEvalErrorLocation(
  ...
  if (errorStack && errorLocator) {
    return extractErrorLocation(errorStack, lineOffset, errorLocator) || UNKNOWN_LOCATION
  } else if (errorStack) {
    // if browser is unsupported try all supported locators until the first success
    return (
      EVAL_LOCATORS.map(locator => extractErrorLocation(errorStack, lineOffset, locator)).find(
        x => x !== undefined
      ) || UNKNOWN_LOCATION
    )
  }
