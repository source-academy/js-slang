...
The thread that calls =concurrent_execute= also continues to execute concurrently with the other threads. This is shown by the $\textit{os}, \textit{pc}, \textit{e}, \textit{rs}$ being in the machine state after the transition arrow.
