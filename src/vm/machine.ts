...
export function show_heap(s: string) {
    ...
    while (i < len) {
        window.console.log("", i.toString() + ": " + HEAP[i].toString() +
           (// TODO is_number(HEAP[i]) &&
               is_node_tag(HEAP[i])
                     ? " (" + node_kind(HEAP[i]) + ")"
                     : ""));
        i = i + 1;
...
function show_heap_value(address: number) {
    window.console.log("", "result: heap node of type = " +
...
M[OpCodes.START] = () =>   { A = 1; // first OS only needs to hold one closure
                             NEW_OS();
                             OS = RES;
                             A = 0;
                             NEW_ENVIRONMENT();
...
M[OpCodes.LDCN] = () =>    { A = P[PC + LDCN_VALUE_OFFSET];
                             NEW_NUMBER();
                             A = RES;
                             PUSH_OS();
                             PC = PC + 2;
...
M[OpCodes.LDCB] = () =>    { A = P[PC + LDCB_VALUE_OFFSET];
                             NEW_BOOL();
                             A = RES;
                             PUSH_OS();
                             PC = PC + 2;
...
M[OpCodes.LDCU] = () =>    { NEW_UNDEFINED();
                             A = RES;
                             PUSH_OS();
                             PC = PC + 1;
                   };

M[OpCodes.PLUS] = () =>    { POP_OS();
...
M[OpCodes.DIV] = () =>     { POP_OS();
                             ...
                             PC = PC + 1;
                             E = E === 0;
                             if (E) { STATE = DIV_ERROR; }
                             if (E) { RUNNING = false; }
                   };

M[OpCodes.POP] = () =>     { POP_OS();
                             PC = PC + 1;
                   };

M[OpCodes.ASSIGN] = () =>  { POP_OS();
                             HEAP[ENV + HEAP[ENV + FIRST_CHILD_SLOT]
                              + P[PC + 1]] = RES;
                             PC = PC + 2;
                   };

M[OpCodes.JOF] = () =>     { POP_OS();
                             A = HEAP[RES + NUMBER_VALUE_SLOT];
                             if (!A) { PC = P[PC + 1]; }
                             if (A) { PC = PC + 2; }
                   };

M[OpCodes.GOTO] = () =>    { PC = P[PC + 1];
                   };

M[OpCodes.LDF] = () =>     { A = P[PC + LDF_MAX_OS_SIZE_OFFSET];
...
M[OpCodes.LD] = () =>      { A = HEAP[ENV + HEAP[ENV + FIRST_CHILD_SLOT]
                                  + P[PC + 1]];
                             PUSH_OS();
                             PC = PC + 2;
                   };

M[OpCodes.CALL] = () =>    { G = P[PC + 1];  // lets keep number of arguments in G
