...
import { BreakValue, ContinueValue, ReturnValue, TailCallReturnValue } from './evaluator-utils'
import { Evaluator, getArgs, apply } from './evaluator-utils'
import { transformLogicalExpression, reduceIf, evaluateBlockStatement } from './evaluator-utils'
...
export const evaluators: {[nodeType: string]: Evaluator<es.Node>} = {
  ...
  FunctionExpression: function*(node: es.FunctionExpression, context: Context) {
    return new Closure(node, env.currentEnvironment(context), context)
  },

  ArrowFunctionExpression: function*(node: es.ArrowFunctionExpression, context: Context) {
    return Closure.makeFromArrowFunction(node, env.currentEnvironment(context), context)
  },
  ...
  FunctionDeclaration: function*(node: es.FunctionDeclaration, context: Context) {
    ...
    const closure = new Closure(node, env.currentEnvironment(context), context)
