...
import {
  Context,
  /*ContiguousArrayElements,*/ Environment,
  Frame,
  Value /*Variant*/
} from '../types'
...
/**
 * Dictionary of functions which handles the logic for how the three registers of the CSE machine
 * change according to the AgendaItem.
 */
const cmdEvaluators: { [command: string]: cmdEvaluator } = {
  ...
  VariableDeclaration: function (command: es.VariableDeclaration, context: Context, agenda: Agenda) {
    ...
    agenda.push({instrType: InstrTypes.ASSIGNMENT, symbol: id.name, const: command.kind === 'const'})
  ...
  Assignment: function (command: IInstr, context: Context, agenda: Agenda, stash: Stash) {
    ...
    // in the CSE machine as well. Was this intended?
