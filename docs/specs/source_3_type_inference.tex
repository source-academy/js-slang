...
\qquad
\Rule{t_{head} \quad t_{tail}}{\Pair(t_\head,t_\tail)}
\qquad
\Rule{t}{\List(t)}
\qquad
\Rule{t}{\Array(t)}
...
&& \hspace{6mm}  [ \texttt{===} \leftarrow \forall((T1, T2) \rightarrow \Bool)] \\
&& \hspace{6mm}  [ \texttt{!==} \leftarrow \forall((T1, T2) \rightarrow \Bool)] \\
...
& $[$ & \texttt{set\_tail} & $\leftarrow$  & $\forall(Pair(T_1,T_2)$ & $\rightarrow$ & $\Undefined)$ & $]$ $\Gamma_0$ \\
& \end{tabular}
...
\[
  ...
  \Sigma_2, \Gamma \vdash E_1^{t_1} : \Sigma_3 \quad
  \Sigma_3, \Gamma \vdash E_2^{t_2} : \Sigma_4 \quad
  (\Sigma_4 \wedge t_1 = t_2) = \Sigma_{5} }{
  \Sigma_0, \Gamma \vdash ( E_0^{t_0}\ \texttt{?}\ E_1^{t_1}\ \texttt{:}\ E_2^{t_2})^t\ :\ \Sigma_5}  
\]
...
\[\Rule{(\Sigma_0 \wedge t = t_1),
          ...
          (\Sigma_1 \wedge t_0 = \Bool) = \Sigma_2 \quad
\Sigma_2, \Gamma \vdash \{S_1\}^{t_1} : \Sigma_3 \quad
\Sigma_3, \Gamma \vdash \{S_2\}^{t_2} : \Sigma_4 \quad
