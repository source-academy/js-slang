...
of JavaScript generate error messages when unexpected values are used as follows.
...
The following table specifies what arguments JavaScript's operators
...
Preceding \texttt{\textbf{?}} and following \texttt{\textbf{if}}, JavaScript only allows
boolean expressions.
...
Array indices in JavaScript are limited to integers $i$ in the range $0 \le i < 2^{32} - 1$.

Pairs in JavaScript are represented by arrays with two elements. Therefore,
\begin{lstlisting}
is_pair([1, 2]);
\end{lstlisting}
and
