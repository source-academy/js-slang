...
\item To simulate high interleaving of concurrent threads, all enqueues and dequeues from \(\textit{tq}\) is done randomly: a thread is inserted into a random index, or popped from a random index in \(\textit{tq}\).
\end{enumerate}
