...
$\texttt{float}$, $\texttt{int}$ or $\texttt{string}$, a type variable $N_i$ can
only represent number types, i.e.
$\texttt{float}$ or $\texttt{int}$, and
...
&& \hspace{2mm} [ || \leftarrow \forall((\Bool, \Bool) \rightarrow \Bool)]\\
&& \hspace{2mm} [ ! \leftarrow \forall(N \rightarrow N)]\\
\end{eqnarray*}
...
&& \hspace{6mm}  [ \texttt{===} \leftarrow \forall((A, A) \rightarrow A)] \\
&& \hspace{6mm}  [ \texttt{!==} \leftarrow \forall((A, A) \rightarrow A)] \\
&& \hspace{6mm}  [ \texttt{>} \leftarrow \forall((A, A) \rightarrow A)]\\
&& \hspace{6mm}  [ \texttt{>=} \leftarrow \forall((A, A) \rightarrow A)]\\
&& \hspace{6mm}  [ \texttt{<} \leftarrow \forall((A, A) \rightarrow A)] \\
...
$\Rule{\textrm{if }t_i = t_i'\textrm{ occurs in }\Sigma}{
  \Sigma(t_i) = \Sigma(t_i')}
\qquad
\Rule{t'' = \Sigma(t) \qquad t''' = \Sigma(t')}{
  \Sigma(t \rightarrow t') = t'' \rightarrow t'''}
$
...
\item If $t$ is $M_i$ and $t'$ is not a type variable and not $\texttt{float}$,
  then stop with a type error.
...
$\texttt{int}$ if $\Sigma(t) = N_i$ for some $i$ and $\Sigma(t)$, otherwise.
